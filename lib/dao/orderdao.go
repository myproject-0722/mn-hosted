...
func (*orderDao) Insert(session *dbsession.DBSession, o *model.Order) (int64, error) {
	result, err := session.Exec("insert into t_order(userid, coinname, mnkey, timetype, price, txid, status) value(?, ?, ?, ?, ?, ?)", o.UserID, o.CoinName, o.MNKey, o.TimeType, o.Price, o.TxID, o.Status)
	if err != nil {
		log.Error(err)
		return -1, err
	}
